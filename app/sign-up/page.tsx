    ...
    <div className="min-h-screen bg-[#050507] flex items-center justify-center px-4 py-24 relative overflow-hidden">
      ...
      <motion.div initial={{opacity:0,y:30,scale:0.97}} animate={{opacity:1,y:0,scale:1}} transition={{duration:0.5}} className="glass-card p-8 w-full max-w-md relative z-10">
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-2 mb-4">
            <svg viewBox="0 0 40 24" className="w-10 h-6" fill="none">
              <defs>
                <linearGradient id="su-l" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#A855F7"/><stop offset="100%" stopColor="#D946EF"/></linearGradient>
                <linearGradient id="su-r" x1="0%" y1="0%" x2="100%" y2="100%"><stop offset="0%" stopColor="#22D3EE"/><stop offset="100%" stopColor="#3B82F6"/></linearGradient>
              </defs>
              <path d="M20 12 C20 12 16 4 10 4 C4 4 0 8 0 12 C0 16 4 20 10 20 C16 20 20 12 20 12Z" fill="url(#su-l)"/>
              <path d="M20 12 C20 12 24 4 30 4 C36 4 40 8 40 12 C40 16 36 20 30 20 C24 20 20 12 20 12Z" fill="url(#su-r)"/>
            </svg>
            <span className="font-display font-bold tracking-widest text-gradient">HACKNFINITY</span>
          </div>
          <h1 className="font-display font-bold text-2xl text-[var(--text-primary)] mb-1">Join Hacknfinity</h1>
          <p className="text-sm text-[var(--text-muted)]">India's Largest Student Tech Community</p>
        </div>
        ...
        <div className="space-y-3 mb-4">
          <input type="text" placeholder="Full Name" className="w-full bg-[var(--bg-base)] border border-[var(--border-subtle)] rounded-xl px-4 py-3 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:border-purple-500/50 transition-colors"/>
          ...
          <input type="text" placeholder="College Name" className="w-full bg-[var(--bg-base)] border border-[var(--border-subtle)] rounded-xl px-4 py-3 text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none focus:border-purple-500/50 transition-colors"/>
          <select className="w-full bg-[var(--bg-base)] border border-[var(--border-subtle)] rounded-xl px-4 py-3 text-sm text-[var(--text-muted)] focus:outline-none focus:border-purple-500/50 transition-colors">
            <option value="">Year of Study</option>
            {["1st Year","2nd Year","3rd Year","4th Year","Masters","PhD"].map(y=><option key={y}>{y}</option>)}
          </select>
        </div>
        <label className="flex items-start gap-3 mb-5 cursor-pointer">
          <input type="checkbox" checked={agreed} onChange={e=>setAgreed(e.target.checked)} className="mt-0.5 accent-purple-500"/>
          <span className="text-xs text-[var(--text-muted)]">I agree to the <button className="text-purple-400 underline">Terms of Service</button> and <button className="text-purple-400 underline">Privacy Policy</button></span>
        </label>
        <button className="btn-primary w-full justify-center py-3 mb-5">Create Account</button>
        <p className="text-center text-sm text-[var(--text-muted)]">Already have an account?{" "}<Link href="/sign-in" className="text-purple-400 hover:text-purple-300 font-medium">Sign in</Link></p>
